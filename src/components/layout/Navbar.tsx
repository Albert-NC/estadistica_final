...
export default function Navbar() {
	...
	// Logo de la Escuela (src/assets/logo_ingAmb.png).
	...
	const handleSearch = (
		...
		if (('key' in e && e.key === 'Enter') || e.type === 'click') {
			if (searchTerm.trim()) {
				window.open(
					`https://www.google.com/search?q=site:unitru.edu.pe+${encodeURIComponent(searchTerm.trim())}`,
					'_blank',
				);
			}
		}
					...
					<div className='flex items-center'>
						<div className='relative group flex items-center'>
							<input
								...
								className='bg-transparent text-white placeholder-gray-400 text-[13px] px-3 py-1 outline-none border-b border-transparent focus:border-gold transition-[border-color,width] w-32 focus:w-48'
							/>
							<Search
				...
				<div className='flex items-center justify-between'>
					...
					<Link to='/' className='flex items-center gap-3 md:gap-4 group py-3'>
						<div className='flex items-center gap-3 md:gap-4 transition-transform group-hover:scale-[1.03]'>
							...
							{schoolLogoOk && (
								<span
									className='block w-px h-10 sm:h-12 md:h-20 bg-gray-300'
									aria-hidden='true'
								/>
							)}
                  ...
                  Si no existe, no se muestra (ni el logo ni el separador). */}
							<img
								src={schoolLogoUrl}
								alt='Escuela Profesional de [Nombre de la Escuela]'
								loading='lazy'
								decoding='async'
						...
						<div className='flex flex-col ml-1 md:ml-3'>
							<span className='font-display font-black tracking-tight leading-[0.95] text-base sm:text-lg md:text-3xl flex flex-col'>
								<span className='text-pucp-blue-dark'>[Nombre</span>
						...
						{NAV_LINKS.map((link, idx) => (
							...
							>
								<NavLink
									...
									aria-expanded={
										link.groups ? openSection === link.name : undefined
									}
